...
class Classifier(): 
    ...
    def _tokenize_function(self, df):
        return self.tokenizer(df["text"], padding='max_length', max_length=512, truncation=True)
    ...
    def fit(self, train_data, valid_data, num_train_epochs, save_path):
        ...
        train_dataset = Dataset.from_pandas(train_data).map(self._tokenize_function, batched=True)
        valid_dataset = Dataset.from_pandas(valid_data).map(self._tokenize_function, batched=True)   
    ...
    def load(ml, model_path):
        ...
        model = AutoModelForSequenceClassification.from_pretrained(os.path.join(model_path, transformer_fname))           
        tokenizer = AutoTokenizer.from_pretrained(os.path.join(model_path, tokenizer_fname)) 
